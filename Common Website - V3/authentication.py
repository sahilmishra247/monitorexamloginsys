...
class Config:
    ...
    FACE_THRESHOLD = 0.90  # Added threshold for face
...
class BiometricAuthenticator(ABC):
    ...
    @abstractmethod
    def extract_features(self, data_bytes: Any) -> np.ndarray:
...
class FaceAuthenticator(BiometricAuthenticator):
    ...
    def extract_features(self, image_data: Any) -> np.ndarray:
        """Extract facial features using face_recognition"""
...
class BiometricManager:
    def __init__(self):
        self.authenticators = {
            ...
            "face": FaceAuthenticator(),  # Adding FaceAuthenticator to the manager
        ...
        for method, authenticator in self.authenticators.items():
            try:
                ...
                if method == "voice":
                    ...
                    available.append(method)  # Now implemented
...
def get_user_registered_methods(user_id):
    ...
    for method__name__, authenticator in biometric_manager.authenticators.items():
        if authenticator.load_features(user_id) is not None:
            registered_methods.append(method__name__)
    ...
    for method__name__, authenticator in biometric_manager.authenticators.items():
        try:
            ...
            if os.path.exists(feature_path):
                ...
                deleted_methods.append(method__name__)
        except Exception as e:
            errors.append(f"Error deleting {method__name__} data: {str(e)}")
